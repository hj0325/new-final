...
function EmojiSelector3D({ onEmojiClick }) {
  const { 
    ...
    emojiScale: { value: 0.5, min: 0.1, max: 3, step: 0.01, label: '크기' },
    emojiYPosition: { value: 1.7, min: -5, max: 5, step: 0.1, label: '수직 위치 (전체 그룹)' },
    emojiZPosition: { value: 3, min: -5, max: 10, step: 0.1, label: '깊이 위치 (Z축)' },
    emojiSpacing: { value: 0.8, min: 0.1, max: 5, step: 0.05, label: '좌우 간격' },
    groupRotationX: { value: 0, min: -180, max: 180, step: 1, label: '그룹 X축 회전 (각도)' },
